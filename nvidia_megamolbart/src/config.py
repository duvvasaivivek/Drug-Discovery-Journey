...
def map_nemo_to_huggingface(nemo_state_dict: dict, vocab_size: int) -> dict:
    ...
    for module in ["encoder", "decoder"]:
        for i in range(6): # 6 layers
            ...
            # Megatron interleaves Q, K, V per head. Shape is (3 * hidden_size, hidden_size) 
            # where the first dim is (num_heads, 3, head_dim)
            qkv_weight = qkv_weight.view(8, 3, 64, 512).permute(1, 0, 2, 3).contiguous().view(3, 512, 512)
            qkv_bias = qkv_bias.view(8, 3, 64).permute(1, 0, 2).contiguous().view(3, 512)
            
            q_w, k_w, v_w = qkv_weight[0], qkv_weight[1], qkv_weight[2]
            q_b, k_b, v_b = qkv_bias[0], qkv_bias[1], qkv_bias[2]
            ...
            if module == "decoder":
                ...
                # Megatron interleaves K, V per head. Shape is (2 * hidden_size, hidden_size)
                # where the first dim is (num_heads, 2, head_dim)
                ckv_weight = ckv_weight.view(8, 2, 64, 512).permute(1, 0, 2, 3).contiguous().view(2, 512, 512)
                ckv_bias = ckv_bias.view(8, 2, 64).permute(1, 0, 2).contiguous().view(2, 512)
                
                ck_w, cv_w = ckv_weight[0], ckv_weight[1]
                ck_b, cv_b = ckv_bias[0], ckv_bias[1]
...
def map_nemo_to_huggingface(nemo_state_dict: dict, vocab_size: int) -> dict:
    ...
    # 3. LM Head
    hf_state_dict["lm_head.weight"] = nemo_state_dict["enc_dec_model.tokens_head.weight"][:vocab_size]
    
    # Missing keys in HF that cause random initialization and break generation
    hf_state_dict["final_logits_bias"] = torch.zeros(1, vocab_size)
    
    # 4. Final Layer Norms (Encoder/Decoder)
    if "enc_dec_model.enc_dec_model.encoder.model.final_layernorm.weight" in nemo_state_dict:
        hf_state_dict["model.encoder.final_layernorm.weight"] = nemo_state_dict["enc_dec_model.enc_dec_model.encoder.model.final_layernorm.weight"]
        hf_state_dict["model.encoder.final_layernorm.bias"] = nemo_state_dict["enc_dec_model.enc_dec_model.encoder.model.final_layernorm.bias"]
        hf_state_dict["model.decoder.final_layernorm.weight"] = nemo_state_dict["enc_dec_model.enc_dec_model.decoder.model.final_layernorm.weight"]
        hf_state_dict["model.decoder.final_layernorm.bias"] = nemo_state_dict["enc_dec_model.enc_dec_model.decoder.model.final_layernorm.bias"]
